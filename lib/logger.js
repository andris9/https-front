...
const { normalizeDomain, unicodeDomain } = require('./tools');
...
// Only a name carrying an A-label or a character outside ASCII is spelled more
// than one way. Everything else is left exactly as it is, which is what keeps
// the access log line, written once per proxied request, from being taken apart
// and rebuilt for nothing.
const spelledOneWay = name => !/(^|\.)xn--|[\x80-\uFFFF]/.test(name);
...
// carries the same name twice.
const spellings = value => {
    const names = typeof value === 'string' ? [value] : value;
    if (!Array.isArray(names) || !names.every(name => typeof name === 'string') || names.every(spelledOneWay)) {
        return null;
    }

    const unicode = names.map(unicodeDomain);
    const alabel = unicode.map(normalizeDomain);
