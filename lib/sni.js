...
const getSNIContext = async servername => {
    ...
    if (config.validate && config.validate.enabled) {
        try {
            let validated = await validateDomain(domain);
            if (!validated) {
                return false;
            }
        } catch (err) {
            log.error('SNI', 'Domain verification failed. domain=%s error=%s', domain, err.message);
            return false;
        }
    }
