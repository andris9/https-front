...
// Blocked for this long after a domain fails validation. Much shorter than the
// error lock, so a domain whose DNS was just corrected is picked up quickly,
// but long enough that repeated requests for a domain that will never validate
// do not repeat the DNS queries behind validateDomain every time.
const BLOCK_RENEW_AFTER_INVALID_TTL = 300;
...
// Blocks renewal attempts for a while after a failure, so that a domain that
// can not be provisioned is not retried on every request.
const blockRenewal = async (redisClient, certKey, domains, ttl) => {
    try {
        await redisClient.set(`${certKey}:lock`, 1, 'EX', ttl);
    } catch (err) {
...
const acquireCert = async opts => {
    ...
    for (let domain of domains) {
        try {
            ...
            // Recorded in Redis rather than in the worker, so that every worker
            // and every instance sharing this certificate pool backs off too.
            await blockRenewal(redisClient, certKey, domains, BLOCK_RENEW_AFTER_INVALID_TTL);
            return false;
        }
    }
    ...
    try {
        ...
        await blockRenewal(redisClient, certKey, domains, BLOCK_RENEW_AFTER_ERROR_TTL);
