...
const acquireCert = async opts => {
    ...
    try {
        ...
        try {
            ...
            logger.error({
                msg: 'Failed to generate CSR file',
                domain: domains.join(', '),
                canDeleteKey: !certificateData || !certificateData.expires || certificateData.expires < new Date(),
                err
            });

            if (!certificateData || !certificateData.expires || certificateData.expires < new Date()) {
                // delete private key entry
                try {
                    let deleted = await redisClient.del(certKey);
                    if (deleted) {
                        logger.info({ msg: 'Deleted domain key from Redis', domain: domains.join(', '), certKey });
                    } else {
                        logger.info({ msg: 'Domain key was not found', domain: domains.join(', '), certKey });
                    }
                } catch (err) {
