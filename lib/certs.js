...
const acquireCert = async opts => {
    ...
    if (await redisClient.exists(`${certKey}:lock`)) {
        ...
        logger.info({ msg: 'Renewal blocked by failsafe lock', domains, certKey });
    ...
    try {
        ...
        if (!privateKey) {
            // generate new key
            logger.info({ msg: 'Provision new private key', domains });
        ...
        if (!acmeAccount) {
            logger.info({ msg: 'Skip certificate renwal, acme account not found', domains });
            return false;
        }
        ...
        logger.info({ msg: 'Generate ACME cert', domains, account: aID });

        const cert = await acme.certificates.create(certificateOptions);
        if (!cert || !cert.cert) {
            logger.error({ msg: 'Failed to generate certificate for %s', domains });
            return cert;
        }
        logger.info({ msg: 'Received certificate from ACME', domains });
        ...
        logger.info({ msg: 'Certificate successfully generated', domains, expires: parsed.validTo });
        ...
        try {
            ...
            logger.info({ msg: 'Redis call failed', key: `${certKey}:lock`, domains, err });
        }

        logger.info({ msg: 'Failed to generate cert', domains, err });
