...
// error the store threw and from the one it recorded instead of throwing.
const recordFailure = async (domain, err) => {
...
const acquireCert = async (domain, stored) => {
    ...
    // Read before the order, to tell a failure from this attempt apart from one
    // left on the record by an earlier one. See below.
    ...
    // The store writes a failure onto the certificate record and leaves it there
    // until an order succeeds, and it hands that record straight back on every
    // path where it did not attempt an order at all: its own failsafe lock, an
    // operation lock it could not take, a certificate that turned out not to be
    // due after it asked the CA. So `lastError` on its own means "something went
    // wrong at some point", which is not what this has to act on.
    //
    // Acting on it anyway is worse than useless here. A single rate limit
    // recorded hours ago would re-arm the pool wide pause every time that one
    // record was read, escalating each time, and the pool would never come back.
    const lastError = certificateData && certificateData.lastError;
    if (lastError && new Date(lastError.time).getTime() >= attemptedAt) {
        // This attempt failed. The store has a failsafe lock of its own for that,
        // but this is what keeps the validation above from running again on the
        // next handshake. The store swallows the error rather than throwing it
        // when there is still a certificate to serve, so this is the only place a
        // rate limit that arrived that way is seen.
        await recordFailure(domain, lastError);
